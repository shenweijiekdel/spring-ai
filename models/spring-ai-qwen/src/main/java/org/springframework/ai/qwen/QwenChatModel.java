...
import org.springframework.ai.qwen.api.QwenApi.ChatCompletion;
import org.springframework.ai.qwen.api.QwenApi.ChatCompletion.Choice;
import org.springframework.ai.qwen.api.QwenApi.ChatCompletionMessage;
import org.springframework.ai.qwen.api.QwenApi.ChatCompletionMessage.ChatCompletionFunction;
import org.springframework.ai.qwen.api.QwenApi.ChatCompletionMessage.ToolCall;
import org.springframework.ai.qwen.api.QwenApi.ChatCompletionRequest;
import org.springframework.ai.qwen.api.common.QwenConstants;
 ...
 * backed by {@link QwenApi}.
 *
 * @author Geng Rong
 */
public class QwenChatModel implements ChatModel {

    private static final Logger logger = LoggerFactory.getLogger(QwenChatModel.class);
    ...
    public QwenChatModel(QwenApi qwenApi, QwenChatOptions defaultOptions,
        ...
        this(qwenApi, defaultOptions, toolCallingManager, retryTemplate, observationRegistry,
    ...
    public ChatResponse internalCall(Prompt prompt, ChatResponse previousChatResponse) {
        ...
        ChatModelObservationContext observationContext = ChatModelObservationContext.builder()
                .prompt(prompt)
                .provider(QwenConstants.PROVIDER_NAME)
                .build();
        ...
        ChatResponse response = ChatModelObservationDocumentation.CHAT_MODEL_OPERATION
                ...
                .observe(() -> {
                    ...
                    ResponseEntity<ChatCompletion> completionEntity = this.retryTemplate
                            .execute(ctx -> this.qwenApi.chatCompletionEntity(request));
                    ...
                    // Current usage
                    QwenApi.Usage usage = completionEntity.getBody().usage();
    ...
    public Flux<ChatResponse> internalStream(Prompt prompt, ChatResponse previousChatResponse) {
        return Flux.deferContextual(contextView -> {
            ...
            Flux<QwenApi.ChatCompletionChunk> completionChunks = this.qwenApi.chatCompletionStream(request);
            ...
            Flux<ChatResponse> chatResponse = completionChunks.map(this::chunkToChatCompletion)
  				...
  				// @formatter:on
                            ...
                            QwenApi.Usage usage = chatCompletion2.usage();
				...
				if (this.toolExecutionEligibilityPredicate.isToolExecutionRequired(prompt.getOptions(), response)) {
					return Flux.defer(() -> {
						...
						if (toolExecutionResult.returnDirect()) {
							...
							// Send the tool execution result back to the model.
                            if (prompt.getOptions() instanceof QwenChatOptions) {
    ...
    private ChatResponseMetadata from(QwenApi.ChatCompletion result, Usage usage) {
        Assert.notNull(result, "Qwen ChatCompletionResult must not be null");
    ...
    private ChatResponseMetadata from(ChatResponseMetadata chatResponseMetadata, Usage usage) {
        Assert.notNull(chatResponseMetadata, "Qwen ChatResponseMetadata must not be null");
        var builder = ChatResponseMetadata.builder()
     ...
     */
    private QwenApi.ChatCompletion chunkToChatCompletion(QwenApi.ChatCompletionChunk chunk) {
        ...
        return new QwenApi.ChatCompletion(chunk.id(), choices, chunk.created(), chunk.model(), chunk.serviceTier(),
                chunk.systemFingerprint(), chunk.usage());
    }

    private DefaultUsage getDefaultUsage(QwenApi.Usage usage) {
    ...
    Prompt buildRequestPrompt(Prompt prompt) {
        ...
        QwenChatOptions requestOptions = ModelOptionsUtils.merge(runtimeOptions, this.defaultOptions,
                QwenChatOptions.class);
    ...
    ChatCompletionRequest createRequest(Prompt prompt, boolean stream) {
        List<ChatCompletionMessage> chatCompletionMessages = prompt.getInstructions().stream().map(message -> {
            if (message.getMessageType() == MessageType.USER || message.getMessageType() == MessageType.SYSTEM) {
                ...
                if (message instanceof QwenAssistantMessage
                        && Boolean.TRUE.equals(((QwenAssistantMessage) message).getPrefix())) {
        ...
        QwenChatOptions requestOptions = (QwenChatOptions) prompt.getOptions();
        ...
        if (!CollectionUtils.isEmpty(toolDefinitions)) {
            request = ModelOptionsUtils.merge(
                    QwenChatOptions.builder().tools(this.getFunctionTools(toolDefinitions)).build(), request,
    ...
    private List<QwenApi.FunctionTool> getFunctionTools(List<ToolDefinition> toolDefinitions) {
        return toolDefinitions.stream().map(toolDefinition -> {
            var function = new QwenApi.FunctionTool.Function(toolDefinition.description(), toolDefinition.name(),
                    toolDefinition.inputSchema());
            return new QwenApi.FunctionTool(function);
        }).toList();
    }

    private ChatOptions buildRequestOptions(QwenApi.ChatCompletionRequest request) {
    ...
    public String toString() {
        return "QwenChatModel [defaultOptions=" + this.defaultOptions + "]";
    }
    ...
    public static final class Builder {
        ...
        private QwenChatOptions defaultOptions = QwenChatOptions.builder()
                .model(QwenApi.DEFAULT_CHAT_MODEL)
                .temperature(0.7)
                .build();
        ...
        public QwenChatModel build() {
            ...
            return new QwenChatModel(this.qwenApi, this.defaultOptions, DEFAULT_TOOL_CALLING_MANAGER,
                    this.retryTemplate, this.observationRegistry, this.toolExecutionEligibilityPredicate);
        }
